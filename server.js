...
// FIXED VERSION: Filters multi-hop routes and validates prices
...
// ===== FIXED: analyzeParaswapRoute function =====
function analyzeParaswapRoute(paraswapResponse) {
  const defaultResult = {
    isMultiHop: false,
    hopCount: 0,
    path: [],
    routeSummary: 'No valid route found'
  };

  if (!paraswapResponse?.priceRoute?.bestRoute?.[0]?.swaps) {
    return defaultResult;
  }

  const swaps = paraswapResponse.priceRoute.bestRoute[0].swaps;
  let tokenPath = [];
  
  if (swaps[0] && Array.isArray(swaps[0].path)) {
    tokenPath = swaps[0].path;
  }

  const hopCount = tokenPath.length - 1;
  const isMultiHop = hopCount > 1;
  
  let routeSummary = `Direct swap`;
  if (isMultiHop) {
    routeSummary = `Multi-hop (${hopCount} steps)`;
  }
...
// ===== FIXED: getParaswapPrice with route analysis =====
async function getParaswapPrice(network, pair, amountIn) {
  try {
    ...
    if (response.data && response.data.priceRoute) {
      ...
      const routeAnalysis = analyzeParaswapRoute(response.data);
      
      console.log(`      Paraswap Route Analysis: ${routeAnalysis.routeSummary}`);
      if (routeAnalysis.isMultiHop) {
        console.log(`      ⚠️  This is a multi-hop route. Likely low liquidity or unreliable price.`);
      }
...
// ===== FIXED: scanArbitrage with multi-hop filtering =====
  ...
  for (const pair of pairsToScan) {
    try {
      ...
      if (uniswapQuotes && uniswapQuotes.length > 0 && paraswapQuote) {
        ...
        // Also filter reverse route if it's multi-hop
        if (paraswapReverseQuote.routeAnalysis && paraswapReverseQuote.routeAnalysis.isMultiHop) {
          console.log(`    ⚠️  SKIPPING: Reverse Paraswap route is multi-hop.`);
          continue;
        }
        ...
        const cycle1Final = paraswapOutput * uniswapReverseOutput;
        const cycle2Final = uniswapOutput * paraswapReverseOutput;
        
        console.log(`    📊 REAL ARBITRAGE CALCULATION:`);
        console.log(`       Cycle 1: 1 ${pair.token0} → ${paraswapOutput.toFixed(6)} ${pair.token1} (Paraswap) → ${cycle1Final.toFixed(6)} ${pair.token0} (Uniswap)`);
        console.log(`       Cycle 2: 1 ${pair.token0} → ${uniswapOutput.toFixed(6)} ${pair.token1} (Uniswap) → ${cycle2Final.toFixed(6)} ${pair.token0} (Paraswap)`);
...
app.get('/api/pairs/:network', (req, res) => {
  ...
  res.json({
    ...
    pairs: TRADING_PAIRS[network].slice(0, 50)
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`\n✅ Scanner: Uniswap V3 ↔ Paraswap V5 with MULTI-HOP FILTERING`);
  console.log(`⚠️  WARNING: All multi-hop routes are now automatically skipped.`);
});
